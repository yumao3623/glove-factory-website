...
export function AuthPanel({ authError, googleEnabled = false }: { authError?: string; googleEnabled?: boolean }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const response = await fetch(`/api/auth/${mode === "signin" ? "sign-in" : mode === "signup" ? "sign-up" : "forgot-password"}`, { method: "POST", credentials: "same-origin", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email: email.trim(), password }) });
  ...
  return <div className="mt-10 max-w-md border border-stone-200 bg-white p-6 shadow-sm">
    ...
    <form className="mt-5 space-y-4" onSubmit={submit}><label className="block text-sm">Email<input required type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="mt-1 block w-full border border-stone-300 px-3 py-2" /></label>{mode !== "forgot" && <label className="block text-sm">Password<input required minLength={mode === "signup" ? 12 : 8} maxLength={128} autoComplete={mode === "signup" ? "new-password" : "current-password"} type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="mt-1 block w-full border border-stone-300 px-3 py-2" /></label>}<button disabled={busy} className="w-full bg-[#0d2b3f] px-4 py-3 text-sm text-white disabled:opacity-50">{busy ? "Please wait…" : mode === "signin" ? "Sign in" : mode === "signup" ? "Create account" : "Send password link"}</button></form>
    {mode === "signin" && <button type="button" onClick={() => { setMode("forgot"); setMessage(""); setPassword(""); }} className="mt-4 min-h-11 text-sm underline underline-offset-4">Forgot password?</button>}
    {mode === "forgot" && <p className="mt-4 text-sm leading-6 text-stone-600">Enter your account email. We will send a single-use link so you can choose a new password.</p>}
    ...
    {mode === "signup" && email && <button type="button" onClick={resendConfirmation} disabled={busy} className="mt-3 text-sm underline">Send account access email</button>}
  </div>;
}
