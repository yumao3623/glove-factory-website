...
3. Endpoint sends a structured notification to `PENDING_PUBLIC_CONTACT` and a neutral confirmation to the buyer only after accepted delivery.
...
Use server-side rate limiting, honeypot and provider-supported CAPTCHA/risk checks; do not rely on client-only validation. Avoid logging message bodies or contact data in analytics. SPF/DKIM/DMARC and a monitored sending domain are launch blockers for email delivery. Exact public recipient remains `PENDING_PUBLIC_CONTACT`.
