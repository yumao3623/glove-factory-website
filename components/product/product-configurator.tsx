...
export function ProductConfigurator({ product }: { product: ApprovedCatalogueProduct }) {
  ...
  return <div className="mt-8 border-y border-stone-300 py-6">
    {colors.length ? <fieldset><legend className="text-xs font-semibold uppercase tracking-[.12em] text-stone-600">Colour direction <span className="font-normal normal-case tracking-normal text-stone-500">{color}</span></legend><div className="mt-3 flex flex-wrap gap-2">{colors.map((value) => <button key={value} type="button" onClick={() => setColor(value)} aria-pressed={color === value} className={`min-h-9 border px-3 text-xs ${color === value ? "border-[#0d2b3f] bg-[#0d2b3f] text-white" : "border-stone-400 bg-white text-stone-700"}`}>{value}</button>)}</div><p className="mt-3 text-xs leading-5 text-stone-500">Other mainstream colours and custom matching can be reviewed with the factory after sample approval.</p></fieldset> : <p className="text-sm leading-6 text-stone-600">Colour direction is available on request; share a reference or swatch in your enquiry.</p>}
