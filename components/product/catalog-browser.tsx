...
const fallbackLabel = (value: string) => LABELS[value.toLowerCase()] ?? value.replaceAll("-", " ").replace(/\b\w/g, (letter) => letter.toUpperCase());
...
export function CatalogBrowser({ products }: Props) {
  ...
  const facets: Facet[] = [
    { key: "family", label: t("catalog.family"), values: valuesFor(products, "family") },
    { key: "material", label: t("catalog.material"), values: valuesFor(products, "material") },
    { key: "length", label: t("catalog.length"), values: valuesFor(products, "length") },
    { key: "fingerStyle", label: t("catalog.fingerStyle"), values: valuesFor(products, "fingerStyle") },
    { key: "color", label: t("catalog.colour"), values: valuesFor(products, "color") },
    { key: "occasion", label: t("catalog.occasion"), values: valuesFor(products, "occasion") },
  ...
  return <section className="bg-[#f7f5f1] px-5 py-8 sm:px-8 lg:px-10 lg:py-12">
    <div className="mx-auto max-w-[1400px]">
      <div className="mb-8 flex flex-wrap items-end justify-between gap-4 border-b border-stone-300 pb-5">
        <div><p className="section-label text-stone-500">{t("catalog.allProducts")}</p><h1 className="mt-2 font-serif text-5xl leading-none sm:text-6xl">{t("catalog.title")}</h1><p className="mt-4 max-w-2xl text-sm leading-6 text-stone-600">{t("catalog.description")}</p></div>
        <div className="flex items-center gap-3 text-sm"><span className="text-stone-600">{filtered.length} {t("catalog.count")}</span><label className="flex items-center gap-2">{t("catalog.sort")}<select aria-label={t("catalog.sort")} value={sort} onChange={(e) => setSort(e.target.value)} className="border-0 border-b border-stone-400 bg-transparent py-2 text-sm"><option value="featured">{t("catalog.featured")}</option><option value="name">{t("catalog.name")}</option></select></label></div>
      </div>
      <div className="mb-8 flex flex-col gap-2 sm:flex-row sm:items-center"><label htmlFor="catalogue-search" className="section-label shrink-0 text-stone-600">{t("catalog.search")}</label><div className="flex min-h-12 flex-1 items-center border border-stone-400 bg-white px-4 focus-within:border-[#0d2b3f] focus-within:ring-1 focus-within:ring-[#0d2b3f]"><input id="catalogue-search" type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder={t("catalog.placeholder")} className="w-full bg-transparent text-base outline-none placeholder:text-stone-400" /><Search size={18} aria-hidden="true" className="text-stone-500" /></div>{search ? <button type="button" onClick={() => setSearch("")} className="min-h-11 px-2 text-sm underline underline-offset-4">{t("catalog.clearSearch")}</button> : null}</div>
      <div className="mb-5 flex items-center justify-between lg:hidden"><button type="button" onClick={() => setMobileOpen(true)} className="inline-flex min-h-11 items-center gap-2 border border-stone-400 px-4 text-sm"><SlidersHorizontal size={16} />{t("catalog.filters")}{active.length ? ` (${active.length})` : ""}</button>{active.length ? <button type="button" onClick={clear} className="text-sm underline">{t("catalog.clearAll")}</button> : null}</div>
      <div className="grid gap-8 lg:grid-cols-[220px_1fr]">
        <aside className={`${mobileOpen ? "fixed inset-0 z-50 block bg-[#f7f5f1] p-6" : "hidden"} lg:static lg:block lg:p-0`}>
          <div className="mb-5 flex items-center justify-between border-b border-stone-300 pb-4"><h2 className="font-serif text-2xl">{t("catalog.filters")}</h2><button type="button" onClick={() => setMobileOpen(false)} className="lg:hidden" aria-label={t("catalog.closeFilters")}><X /></button></div>
          ...
          <button type="button" onClick={clear} className="text-sm underline underline-offset-4">{t("catalog.clearAll")}</button>
        </aside>
        <div>
          ...
          </> : <div className="border-y border-stone-300 py-20 text-center"><h2 className="font-serif text-3xl">{t("catalog.noMatch")}</h2><p className="mt-3 text-stone-600">{t("catalog.tryRemoving")}</p><button type="button" onClick={clear} className="mt-6 underline">{t("catalog.clearAll")}</button></div>}
        </div>
      </div>
    </div>
  </section>;
