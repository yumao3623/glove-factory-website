...
export function CollectionPage({ config, products }: { config: CollectionConfig; products: readonly ApprovedCatalogueProduct[] }) {
  ...
  return <main id="main-content" tabIndex={-1} className="overflow-hidden"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(breadcrumbSchema) }} />
    ...
    <section id="rfq" className="bg-black text-white"><div className="mx-auto grid max-w-[1280px] gap-10 px-5 py-16 sm:px-8 lg:grid-cols-[.75fr_1.25fr] lg:px-10 lg:py-24"><div><p className="section-label text-stone-400">Enquiry</p><h2 className="mt-3 max-w-md font-serif text-4xl leading-[1.04] sm:text-5xl">Start with your sourcing brief.</h2><p className="mt-5 max-w-sm leading-7 text-stone-300">Share the family, visual direction and requirements relevant to your range.</p></div><div className="rfq-on-dark"><RfqForm /></div></div></section>
  </main>;
}
