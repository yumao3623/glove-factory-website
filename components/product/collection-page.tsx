...
function Breadcrumb({ title, dark = false }: { title: string; dark?: boolean }) {
  ...
  return <nav aria-label="Breadcrumb" className={`text-sm ${colors}`}><Link href="/" className="focus-visible:outline-none focus-visible:ring-2"><LocalizedText k="common.home" fallback="Home" /></Link><span aria-hidden="true"> | </span><Link href="/products/" className="focus-visible:outline-none focus-visible:ring-2"><LocalizedText k="common.products" fallback="Products" /></Link><span aria-hidden="true"> | </span><span aria-current="page">{title}</span></nav>;
}
...
function CollectionHero({ config, product }: { config: CollectionConfig; product: ApprovedCatalogueProduct | undefined }) {
  ...
  return <section className={dark ? "bg-black text-white" : config.layout === "kids" ? "bg-[#f0e7df]" : "bg-white"}><div className="mx-auto max-w-[1280px] px-5 py-8 sm:px-8 lg:px-10 lg:py-12"><Breadcrumb title={config.title} dark={dark} /><div className="grid gap-10 py-12 sm:py-16 lg:grid-cols-12 lg:items-center lg:gap-12 lg:py-16"><div className="lg:col-span-5"><p className={dark ? "section-label text-stone-400" : "section-label"}>{config.eyebrow}</p><h1 className="mt-3 max-w-[8ch] font-serif text-[clamp(3.4rem,7vw,6.4rem)] leading-[.87]">{config.title}</h1><p className={dark ? "mt-6 max-w-[30ch] leading-7 text-stone-300" : "mt-6 max-w-[30ch] leading-7 text-stone-600"}>{config.intro}</p><Button asChild size="lg" className={dark ? "mt-8 min-h-11 rounded-none bg-white px-6 text-black hover:bg-stone-200" : "mt-8 min-h-11 rounded-none bg-black px-6 hover:bg-stone-800"}><a href="#collection"><LocalizedText k="common.exploreCollection" fallback="Explore the collection" /> <ArrowRightIcon data-icon="inline-end" /></a></Button></div>{product ? <EditorialImage src={approvedProductImage(product.primaryImage)} alt={product.primaryImage.altText} priority className="aspect-[4/5] lg:col-span-7" imageClassName="object-cover object-center" sizes="(min-width: 1024px) 52vw, 100vw" /> : null}</div></div></section>;
}

function CollectionRange({ config, products }: { config: CollectionConfig; products: readonly ApprovedCatalogueProduct[] }) {
  return <section id="collection" className={config.layout === "kids" ? "bg-[#f8f5f1]" : "bg-[#f6f5f2]"}><div className="mx-auto max-w-[1280px] px-5 py-14 sm:px-8 lg:px-10 lg:py-24"><div className="flex flex-col justify-between gap-6 border-b border-stone-300 pb-6 lg:flex-row lg:items-end"><div><p className="section-label"><LocalizedText k="common.collection" fallback="Collection" /></p><h2 className="mt-3 max-w-[17ch] font-serif text-3xl leading-[1.04] sm:max-w-xl sm:text-4xl sm:leading-tight">{config.rangeHeading}</h2></div><p className="max-w-sm text-sm leading-6 text-stone-600">{config.rangeCopy}</p></div><div className="mt-10">{products.map((product) => <ProductCard key={product.id} product={product} />)}</div></div></section>;
}

function CollectionProcurement({ config, rfqEnabled }: { config: CollectionConfig; rfqEnabled: boolean }) {
  return <section className={config.layout === "opera" ? "bg-[#dfe3e2]" : "bg-white"}><div className="mx-auto grid max-w-[1280px] gap-8 px-5 py-14 sm:px-8 lg:grid-cols-12 lg:px-10 lg:py-20"><div className="flex flex-col justify-center lg:col-span-6 lg:col-start-4"><p className="section-label"><LocalizedText k="common.sourcingContext" fallback="Sourcing context" /></p><h2 className="mt-3 font-serif text-4xl leading-[1.04] sm:text-5xl">{config.procurementHeading}</h2><p className="mt-5 leading-7 text-stone-600">{config.procurementCopy}</p><div className="mt-7 border-y border-stone-300 text-sm"><div className="flex justify-between gap-5 py-3"><span><LocalizedText k="common.materialComposition" fallback="Material and composition" /></span><span className="shrink-0 text-stone-500"><LocalizedText k="common.reviewOnly" fallback="Review only" /></span></div><div className="flex justify-between gap-5 border-t border-stone-300 py-3"><span><LocalizedText k="common.dimensionsSizing" fallback="Dimensions and sizing" /></span><span className="shrink-0 text-stone-500"><LocalizedText k="common.reviewOnly" fallback="Review only" /></span></div><div className="flex justify-between gap-5 border-t border-stone-300 py-3"><span><LocalizedText k="common.sampleTiming" fallback="Sample and timing" /></span><span className="shrink-0 text-stone-500"><LocalizedText k="common.reviewOnly" fallback="Review only" /></span></div></div><div className="mt-5 flex flex-wrap gap-x-5 gap-y-2 text-sm"><Link href="/guides/materials/" className="underline underline-offset-4"><LocalizedText k="common.compareMaterials" fallback="Compare materials" /></Link><Link href="/guides/size-guide/" className="underline underline-offset-4"><LocalizedText k="common.prepareMeasurements" fallback="Prepare measurements" /></Link><Link href="/custom-manufacturing/" className="underline underline-offset-4"><LocalizedText k="common.discussCustom" fallback="Discuss custom work" /></Link></div><Button asChild variant="link" size="lg" className="mt-4 w-fit rounded-none px-0 text-black underline-offset-4"><a href="#rfq">{rfqEnabled ? <LocalizedText k="common.startEnquiry" fallback="Start an enquiry" /> : <LocalizedText k="common.viewPreview" fallback="View Preview Status" />} <ArrowRightIcon data-icon="inline-end" /></a></Button></div></div></section>;
}
...
export const costumeCollection: CollectionConfig = { family: "costume-gloves", title: "Costume | Stage Gloves", eyebrow: "Performance and costume sourcing", intro: "A supported collection of non-licensed costume and stage glove directions.", layout: "costume", rangeHeading: "A focused stage and costume direction.", rangeCopy: "Use these non-licensed visual references as a starting point for a supported performance or costume brief.", procurementHeading: "Keep the brief specific.", procurementCopy: "Share the intended performance context, construction, colour and decoration requirements for review." };
export const armSleevesCollection: CollectionConfig = { family: "arm-sleeves", title: "Arm Sleeves | Occasion Accessories", eyebrow: "Sleeve and accessory sourcing", intro: "A separate direction for fingerless sleeves and coordinated occasion accessories.", layout: "accessories", rangeHeading: "A sleeve-led accessory direction.", rangeCopy: "Review long, fingerless silhouettes as a starting point for bridal, formalwear and costume accessory programmes.", procurementHeading: "Specify the proportion.", procurementCopy: "Bring sleeve length, opening, trim, colour and volume requirements into the sourcing conversation." };
